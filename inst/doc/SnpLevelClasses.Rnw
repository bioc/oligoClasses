...
%\item \Robject{SnpSet}: a class for genotype calls and confidence
%  scores.  AssayData elements include \Robject{call} and
%  \Robject{callProbability}.
...
\item \Robject{oligoSnpSet}: class containing both genotype calls and
  total copy number estimates, as well as the corresponding confidence
  scores.  AssayData elements include include the union of elements in
  \Robject{SnpSet}, \code{copyNumber}, and \code{cnConfidence}.
\end{itemize}
